...
void MainWindow::privilage() {
    ...
    // Vérifier si le rôle est admin pour donner un accès complet
    if (role == "admin") {
        // Activer tous les onglets pour l'administrateur
        for (int i = 0; i < ui->tabWidget_6->count(); ++i) {
            ui->tabWidget_6->setTabEnabled(i, true);
        }
        qDebug() << "Accès complet activé pour l'administrateur.";
    } else {
...
void MainWindow::readData() {
    QByteArray data = a.read_from_arduino(); // Lire la donnée depuis l'Arduino (0 ou 1)
    QString movementStatus = QString::fromStdString(data.toStdString());

    // Insérer la donnée dans la table 'depot'
    QSqlQuery query;
    query.prepare("INSERT INTO depot (id, statut) VALUES (:id, :statut)");
    query.bindValue(":statut", movementStatus);

    if (!query.exec()) {
        qDebug() << "Erreur lors de l'insertion dans la base de données : " << query.lastError().text();
    } else {
        qDebug() << "Données insérées : Mouvement = " << movementStatus;
    }
}
void MainWindow::readSerialData()
{
    while (serial->canReadLine()) {
        ...
        QString motion = QString::fromUtf8(data);

        qDebug() << "Données reçues :" << motion;


        if (motion == "1") {
            sendToArduino("1");
            qDebug() << "Message 'oui' envoyé à l'Arduino";
        }

        // Convertir les valeurs "0" et "1" en chaînes de caractères
        QString movementStatus;
        if (motion == "1") {
            movementStatus = "qt Mouvement détecté";
        } else if (motion == "0") {
            movementStatus = "qt Pas de mouvement";
        } else {
            movementStatus = "Mouvement détecté"; // Gérer les données inattendues
        }

        // Sauvegarder le mouvement sous forme de chaîne dans la base de données
        saveToDatabase(movementStatus);


    }
}

void MainWindow::saveToDatabase(const QString &etat) {
    // Insérer uniquement l'état (en texte) dans la base de données
    QSqlQuery query;
    query.prepare("INSERT INTO depot (statut) VALUES (:statut)");
    query.bindValue(":statut", etat);

    if (query.exec()) {
        qDebug() << "Mouvement enregistré : Etat = " << etat;
    } else {
...
void MainWindow::setupSerialPort() {
    ...
    serial->setPortName("COM5");  // Remplacez "COM5" par le port série de votre Arduino
...
void MainWindow::sendToArduino(const QString &data)
{
    // Vérifie si le port série est ouvert et prêt à écrire
    if (serial && serial->isOpen()) {
        // Envoie la donnée à l'Arduino en UTF-8, suivie d'un saut de ligne
        serial->write(data.toUtf8() + '\n');
        qDebug() << "Donnée envoyée à l'Arduino :" << data;
    } else {
        qDebug() << "Erreur : Port série non ouvert ou indisponible.";
    }
}
////////////
void MainWindow::readFromArduino2()
{
    ...
    {
        QSqlQuery sql;
        sql.prepare("UPDATE matieres SET ETAT='Haut' ");
        sql.exec();
        QByteArray byte;
        byte.append("O");
    ...
    {
        QSqlQuery sql;
        sql.prepare("UPDATE matieres SET ETAT='Normal' ");
        sql.exec();
    }
}
