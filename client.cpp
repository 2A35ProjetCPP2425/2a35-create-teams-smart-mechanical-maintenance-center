...
bool client::ajouter(){
    ...
    query.prepare("insert into client (code_client , nom , prenom , cin , adresse , pays , tel , mail)" "values (:code_client, :nom, :prenom, :cin, :adresse, :pays, :tel, :mail)");
...
void client::displayRegionStatistics(QGraphicsView *pieView) {
    int afrique=0, europe = 0, asie = 0, amerique = 0, australie=0;

    // Querying the database for client counts by region based on 'pays' column
    QSqlQuery query;

    if (query.exec("SELECT COUNT(*) FROM client WHERE pays IN ('Tunisie')")) {
        if (query.next()) {
            afrique = query.value(0).toInt();
        }
    } else {
        qDebug() << "Erreur lors de la récupération des données pour l'Afrique:" << query.lastError().text();
    }

    if (query.exec("SELECT COUNT(*) FROM client WHERE pays IN ('France', 'Espagne', 'Allemagne', 'Italie', 'UK')")) {
        if (query.next()) {
            europe = query.value(0).toInt();
        }
    } else {
        qDebug() << "Erreur lors de la récupération des données pour l'Europe:" << query.lastError().text();
    }

    if (query.exec("SELECT COUNT(*) FROM client WHERE pays IN ('Emirates')")) {
        if (query.next()) {
            asie = query.value(0).toInt();
        }
    } else {
        qDebug() << "Erreur lors de la récupération des données pour l'Asie:" << query.lastError().text();
    }

    if (query.exec("SELECT COUNT(*) FROM client WHERE pays IN ('US', 'Canada')")) {
        if (query.next()) {
            amerique = query.value(0).toInt();
        }
    } else {
        qDebug() << "Erreur lors de la récupération des données pour l'Amerique:" << query.lastError().text();
    }

    if (query.exec("SELECT COUNT(*) FROM client WHERE pays IN ('Australia')")) {
        if (query.next()) {
            australie = query.value(0).toInt();
        }
    } else {
        qDebug() << "Erreur lors de la récupération des données pour l'Australie:" << query.lastError().text();
    }
    qDebug() << "Clients en Afrique:" << afrique;
    qDebug() << "Clients en Europe:" << europe;
    qDebug() << "Clients en Asie:" << asie;
    qDebug() << "Clients en Amérique:" << amerique;
    qDebug() << "Clients en Australie:" << australie;

    // Creating the chart with the collected data
    QPieSeries *series = new QPieSeries();
    if (afrique > 0) series->append("Afrique", afrique);
    if (europe > 0) series->append("Europe", europe);
    if (asie > 0) series->append("Asie", asie);
    if (amerique > 0) series->append("Amérique", amerique);
    if (australie > 0) series->append("Australie", australie);

    if (series->slices().isEmpty()) {
        series->append("Aucune donnée", 1);  // Placeholder if no data
    }

    QChart *chart = new QChart();
    chart->addSeries(series);
    chart->setTitle("Distribution des clients par région");
    chart->setMargins(QMargins(0, 0, 0, 0)); // Optional: to remove margins around the chart

    // Customize labels to show percentages
    for (QPieSlice *slice : series->slices()) {
        slice->setLabelVisible(true);
        slice->setLabel(QString("%1: %2%").arg(slice->label()).arg(slice->percentage() * 100, 0, 'f', 1));
    }
    ...
    // Set chartView size to match pieView's size for proper display
    chartView->setFixedSize(pieView->width(), pieView->height());

    // Create a graphics scene and add the chartView to it
    QGraphicsScene *scene = new QGraphicsScene();
    scene->addWidget(chartView);

    // Set the scene to the pieView and display it
    pieView->setScene(scene);
    pieView->show();
}
